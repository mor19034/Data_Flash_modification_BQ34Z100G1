...
uint16_t BQ34Z100_G1::getNewPackConfiguration() {
  ...
  selectDataFlashClass(PACK_CONFIG_SUBCLASS); // Seleccionar la subclase de configuración de pack
  selectDataFlashBlock(0x00); // Seleccionar el primer bloque de la subclase
  oldPackConfigMSB = readBlockDataByte(PACK_CONFIG_OFFSET); // Leer el byte alto de la configuración de pack
  oldPackConfigMSB | 256*readBlockDataByte(PACK_CONFIG_OFFSET); //oldPackConfigMSB | VOLTSEL_BIT; // Establecer el bit de selección de voltaje y devolver el nuevo byte alto de la configuración de pack
